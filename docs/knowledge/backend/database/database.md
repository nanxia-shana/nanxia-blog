...
数据库设计的好坏，直接决定了项目的开发效率、运行性能、维护成本、扩展能力。数据库设计是系统架构的基石，设计得好，后期维护省心、扩展容易、性能扛得住；设计得烂，加字段要改几十张表、查个数据要等半分钟、上线三天就得重构。
...
最后，数据库设计没有"标准答案"，适合业务的就是最好的。别为了追求"规范"而过度设计，也别图省事随便建表。多想想一年后数据量会变成什么样，这个表能不能撑住。
