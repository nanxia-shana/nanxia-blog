...
在日常前端开发中，我们最常用的就是 HTTP 请求了——点击按钮发个 POST，拉取数据发个 GET，然后断开连接。

在大部分场景下这完全够用，但总有那么些场景，HTTP 就显得力不从心了
例如：
- 做个聊天室，总不能让用户每隔一秒刷新页面吧？
- 在线协作文档，对方改了内容怎么实时同步过来？
- 股票行情、实时监控，延迟个几秒体验就差很多

这时候我们就需要「实时通信」技术了。今天这篇文章，我们来说说前端常用的四种实时通信方案：短轮询、长轮询、SSE、WebSocket。
...
::: warning 踩坑提醒
为什么说短轮询只适合用户量特别小的内部系统呢？假如有一个项目使用了段轮询，上线后你就会发现服务器 QPS 可能直接翻了十几倍。可以算一笔账：若同时有 1000 个用户，每 3 秒轮询一次，就是每秒 300+ 请求，而且可能其中 90% 以上的请求都只是返回一个空数组，得不偿失。
:::
...
::: danger 踩坑提醒
SSE 有个常见的坑，因为很多反向代理（比如 Nginx）默认会缓冲响应，导致消息被卡住不推送。所以一定要记得在代理配置里关掉缓冲，或者让服务端发送 `X-Accel-Buffering: no` 响应头。而且这个坑恶心就恶心在开发环境时通常直连服务端没有代理，所以 SSE 工作正常，但是上了 Nginx 后，SSE 莫名其妙不工作了，而且没有报错，就是收不到消息，难以发现。
:::

::: tip 小提示
我们现在使用的绝大多数网页版 AI 对话，流畅的逐字回复体验都来自 SSE 技术。
:::
...
::: tip 小提示
实际项目中很少直接用原生 WebSocket，一般都会用封装好的库，比如 [Socket.IO](https://socket.io/)。它不仅封装了重连、心跳、消息确认这些基础能力，还支持降级策略——如果 WebSocket 连不上，会自动降级成长轮询，兼容性极高。
:::
